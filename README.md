...
**Demo Videos:** 📺 [All Demo Videos](docs/demo-videos.md) | 🎥 [AWS Show & Tell](https://www.youtube.com/watch?v=dk0qVukHLGU) | ⭐ [MCP Registry CLI Demo](https://github.com/user-attachments/assets/98200866-e8bd-4ac3-bad6-c6d42b261dbe) | [Full End-to-End Functionality](https://github.com/user-attachments/assets/5ffd8e81-8885-4412-a4d4-3339bbdba4fb) | [OAuth 3-Legged Authentication](https://github.com/user-attachments/assets/3c3a570b-29e6-4dd3-b213-4175884396cc) | [Dynamic Tool Discovery](https://github.com/user-attachments/assets/cee25b31-61e4-4089-918c-c3757f84518c) | [Agent Skills](https://github.com/user-attachments/assets/5d1f227a-25f8-480d-9ff9-acba2498844b) | [Virtual MCP Servers](https://app.vidcast.io/share/954e6296-f217-4559-8d86-88cec25af763) | [Slide Deck](docs/slides/mcp-gateway-registry-presentation.pdf)
...
- **Per-User Egress Auth for Third-Party SaaS MCP Servers (3LO)** - The registry now supports authentication-protected third-party SaaS MCP servers such as **GitHub, Slack, and Atlassian** (and more). Each user connects their own account once through the Registry UI; the gateway runs the OAuth (3LO) flow **out of band** from the coding assistant, vaults the resulting per-user token in a secrets vault (OpenBao for self-hosted/EKS + local dev, AWS Secrets Manager for ECS), and injects it as the `Authorization` header on egress. The coding assistant only ever presents its own ingress credential (via DCR today, CIMD next) — **the third-party token is never stored on the user's laptop**. Tokens are keyed by a per-user, per-server 4-tuple `(auth_method, user_id, provider, server_path)`, lazily refreshed, and never copied to the app database. Connect from the UI's **Connected Accounts** page (a "Click here to connect your accounts" link on the MCP Servers page), then add the server to Claude Code / Codex as usual. Off by default (`EGRESS_AUTH_ENABLED=true`). Token-exchange (OBO) and vault-PAT modes are designed as follow-ons. **Why this matters for enterprise security & governance:** the **gateway owns the auth to every third-party server** — users never store third-party tokens locally, and clients only need **network connectivity to the gateway, not to each MCP server**. That collapses egress to a single, auditable choke point: one place to enforce access control, rotate/revoke credentials, log every call, and reach the internet, instead of scattering long-lived tokens and outbound network paths across every developer laptop. [How it works (design + diagrams)](docs/design/egress-auth-design.md) | [FAQ: Registering GitHub, Slack, Atlassian](docs/faq/registering-third-party-mcp-servers.md) | [Egress Credential Vault](docs/egress-credential-vault.md)

- **Agentic Resource Discovery (ARD) — full spec support** - The registry implements the [ARD v1.0 spec](https://github.com/ards-project/ard-spec) end to end as a **Publisher**, a **Registry**, and a **federating peer**, so any ARD-aware client or registry can discover, search, and cross-reference its assets through vendor-neutral interfaces. **Publish:** a public `/.well-known/ai-catalog.json` renders every public MCP server, A2A agent, and skill as a conformant ARD `catalogEntry` (domain-anchored URNs, per-asset IANA media types, `https` `trustManifest`). **Serve:** `POST /api/ard/search` + `GET /api/ard/agents` expose hybrid semantic search/browse over the ARD contract — JWT-required, access-scoped, paginated, with the ARD `{errorCode, message}` envelope. **Federate:** ingest other registries' `ai-catalog.json` catalogs (servers, agents, skills) into a unified index and honor `federation=none|auto|referrals` on search; ingestion is SSRF-guarded (https-only, post-DNS private/metadata-IP block, same-domain recursion, streamed 5 MB cap) with domain-anchored trust that anchors each entry's URN to the configured source. Discovered assets are read-only, never re-published, and link back to the source descriptor ("view at source") in the UI and search results. Off by default; managed via **Settings → Federation** and the `ard-*` CLI commands. ([#1294](https://github.com/agentic-community/mcp-gateway-registry/issues/1294), [#1295](https://github.com/agentic-community/mcp-gateway-registry/issues/1295), [#1296](https://github.com/agentic-community/mcp-gateway-registry/issues/1296), [#1318](https://github.com/agentic-community/mcp-gateway-registry/pull/1318), [#1336](https://github.com/agentic-community/mcp-gateway-registry/pull/1336)) [ARD Guide](docs/ard.md)
...
- **File Storage Backend and FAISS Search Removed (Breaking Change)** - The legacy file-based storage backend and its in-memory FAISS vector search engine have been removed. DocumentDB / MongoDB is now the only storage backend and the only semantic search engine, which simplifies the codebase (a single storage path, no backend-selection branching) and drops the `faiss-cpu` native dependency. The legacy `scopes.yml` authorization file is also removed: scopes now live exclusively in the `mcp_scopes` collection, seeded from JSON files in `scripts/` at init time and managed via the scope API. `STORAGE_BACKEND=file` is now rejected at startup with a migration message; the default is `mongodb-ce`. Existing file-backend deployments must move to a MongoDB-compatible backend and run [`scripts/migrate-file-to-mongodb.py`](scripts/migrate-file-to-mongodb.py) (migrates servers, agents, federation config, and peer state; security scans are regenerable and not migrated). `torch` and `sentence-transformers` are retained, as they power the default DocumentDB embeddings provider. ([#452](https://github.com/agentic-community/mcp-gateway-registry/issues/452), [#1285](https://github.com/agentic-community/mcp-gateway-registry/issues/1285), [#1292](https://github.com/agentic-community/mcp-gateway-registry/pull/1292)) [Storage Architecture](docs/design/storage-architecture-mongodb-documentdb.md)
...
- **Group-Restricted Agent Visibility** - Agent publishers can now restrict which IdP groups can see their agent by setting `visibility: "group-restricted"` and specifying `allowedGroups` at registration time, without needing an admin to change IAM scopes. Works as a second filter on top of the existing IAM group scope layer: users must pass both the IAM scope check and the allowed_groups check. Nginx forwards JWT group claims via X-Groups header, the list endpoint enforces group filtering for all non-admin users, and the CLI supports `--allowed-groups` for both registration and filtering. Frontend registration and edit forms include a Visibility dropdown and Allowed Groups input. Compatible with all supported IdPs (Keycloak, Entra ID, Cognito, Okta, Auth0, PingFederate). ([#883](https://github.com/agentic-community/mcp-gateway-registry/issues/883), [#922](https://github.com/agentic-community/mcp-gateway-registry/issues/922)) [Full Guide](docs/agent-visibility-and-group-access.md) | [FAQ](docs/faq/group-restricted-agent-visibility.md)
...
- **🔐 Keycloak Integration** - Enterprise authentication with AI agent audit trails and group-based authorization. [MCP Client Guide](docs/keycloak-mcp-clients.md) | [Agent M2M & Operations Guide](docs/keycloak-agent-m2m.md)
...
|------------------|-------------------|------------------------|
| [Complete Setup Guide](docs/complete-setup-guide.md)<br/>**NEW!** Step-by-step from scratch on AWS EC2 | [Authentication Guide](docs/auth.md)<br/>OAuth and identity provider integration | [AI Coding Assistant Registry Integration](docs/ai-coding-assistant-registry-integration.md)<br/>**NEW!** One-command DCR + OAuth setup for Claude Code and Claude.ai<br/>[AI Coding Assistants Setup](docs/ai-coding-assistants-setup.md)<br/>VS Code, Cursor, Claude Code static-token configuration |
...
| [Keycloak: Agent M2M & Operations](docs/keycloak-agent-m2m.md)<br/>Service accounts, ops, agent audit trails<br/>[Keycloak: MCP Client Guide](docs/keycloak-mcp-clients.md)<br/>DCR and OAuth flows for end-user MCP clients | [Token Refresh Service](docs/token-refresh-service.md)<br/>Automated token refresh and lifecycle management | [MCP Registry CLI](docs/mcp-registry-cli.md)<br/>Command-line client for registry management |
...
- **[#1063 - CDK-based Deployment](https://github.com/agentic-community/mcp-gateway-registry/issues/1063)** **PLANNED** (1.24.5)
  CDK deployment option alongside the existing Terraform module for AWS-native installs.

- **[#1020 - Rename `mcpgw` to `airegistry-tools`](https://github.com/agentic-community/mcp-gateway-registry/issues/1020)** **BACKLOG**
